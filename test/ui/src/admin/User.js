    ...
    render() {
        ...
        if (this.state.user.ID) {
            this.state.roles.forEach((role, i) => {
                roleList.push(
                    <li key={i}>{role}<button type="button" onClick={() => this.handleRoleDelete(i)}>-</button></li>
                );
            ...
            roles.forEach(role => {
                roleOptions.push(
                    <option key={roleOptions.length + 1}>{role}</option>
                );
            });
            roleList.push(
                <li key="new_role">
                    <select onChange={this.handleRoleAddSelection} value={this.state.selectedRole}>{roleOptions}</select>
                    <button onClick={this.handleRoleAdd} type="button">+</button>
                </li>
            );
            rolesField = (
                <Fragment>
